...
class UserShell extends Shell
{
    ...
    public function main()
    {
        ...
        if ($continue !== 'y') {
            return $this->abort('User not saved.');
        }
        $this->out('');
        $this->hr();
        $table = TableRegistry::getTableLocator()->get($this->params['table']);
        $entity = $table->newEntity($data, ['validate' => false]);
        if (!$table->save($entity)) {
            return $this->abort(sprintf('User could not be inserted: %s', print_r($entity->getErrors(), true)));
        }
    ...
    public function getOptionParser()
    {
        ...
        $parser->setDescription('The User shell can create a user on the fly for local development.');
